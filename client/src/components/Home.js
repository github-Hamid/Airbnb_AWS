...
async function getListByPrice_type(price, type) {
  const response = await fetch(
    `/Airbnb/api/price&type?price=${price}&type=${type}`
  );
...
async function getListByPrice(price) {
  const response = await fetch(`/Airbnb/api/price/${price}`);
  const data = await response.json();
  console.log("data:", data);
  if (response.status === 200) return data;
}
...
async function getListByType(type) {
  console.log("by type:");
  const response = await fetch(`/Airbnb/api/type/${type}`);
  const data = await response.json();
  console.log("data:", data);
  if (response.status === 200) return data;
}
...
async function getListByCountry(country) {
  ...
  const response = await fetch(`/Airbnb/api/country/${country}`);
  const data = await response.json();
  if (response.status === 200) return data;
}
...
async function getListByPrice_country(price, country) {
  ...
  const response = await fetch(
    `/Airbnb/api/price&country?price=${price}&country=${country}`
  );
  const data = await response.json();
  if (response.status === 200) return data;
}
...
async function getListByType_country(type, country) {
  ...
  const response = await fetch(
    `/Airbnb/api/type&country?type=${type}&country=${country}`
  );
  const data = await response.json();
  if (response.status === 200) return data;
}
...
async function getListByPrice_type_country(price, type, country) {
  ...
  const response = await fetch(
    `/Airbnb/api/price&type&country?price=${price}&type=${type}&country=${country}`
  );
  const data = await response.json();
  if (response.status === 200) return data;
}
...
function Home() {
  ...
  function submitClicked(e) {
    ...
    if (checked.length === 2) {
      ...
      let status =
        sortedArray[0] == "country" ? (sortedArray[1] == "type" ? 1 : 2) : 3;
      console.log("Sorted array:", sortedArray, "status:", status);
      // eslint-disable-next-line default-case
      switch (status) {
        case 1:
          getListByType_country(type, country).then((data) => {
            setList(data);
          });
          break;
        case 2:
          getListByPrice_country(price, country).then((data) => {
            setList(data);
          });
          break;
        case 3:
          getListByPrice_type(price, type).then((list) => {
            setList(list);
          });
          break;
      }
      ...
      switch (name) {
        case "price":
          getListByPrice(price).then((data) => {
            setList(data);
          });
          break;
        case "type":
          getListByType(type).then((data) => {
            setList(data);
          });
          break;
        case "country":
          getListByCountry(country).then((data) => {
            setList(data);
          });
          break;
      }
    } else {
      getListByPrice_type_country(price, type, country).then((data) => {
        setList(data);
      });
    }
  }
  ...
  useEffect(() => {
    fetch("/Airbnb/api").then((response) => {
      response.json().then((data) => {
        setList(data);
      });
    });
  ...
  return (
    <div className="container">
      <div className="container__div">
        ...
        />
        <label
          htmlFor="checkboxFilterPrice"
          style={{ marginRight: "3px", marginLeft: "8px" }}
        >
          Price
        </label>
        <input
          name="checkboxFilterPrice"
          type="checkbox"
          onChange={handleCheckbox}
        />
        <label
          htmlFor="checkboxFilterCountry"
          style={{ marginRight: "3px", marginLeft: "8px" }}
        >
          Country
        </label>
        <input
          name="checkboxFilterCountry"
          type="checkbox"
          onChange={handleCheckbox}
        />
        <input
          id="inputType"
          className="container__div--unchecked"
        ...
        <button
          id="submitBtn"
          className="container__div__btn container__div--unchecked"
      ...
      {lists && (
        <Slider style={{ zIndex: "0" }} {...settings} className="slider">
          {lists.list.map((item, i) => {
            return (
              <div key={i}>
                <Card btnClicked={handleShowMoreBtn} data={item} />
              </div>
            );
          })}
        </Slider>
